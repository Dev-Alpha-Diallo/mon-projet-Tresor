        ...
        <div class="flex items-center space-x-3">
            ...
            <div class="relative">
                <select onchange="window.location.href = this.value" 
                    ...
                    <option value="{{ route('admin.factures.index') }}">Toutes les factures</option>
                    <option value="{{ route('admin.factures.index', ['filter' => 'impayees']) }}" {{ request('filter') == 'impayees' ? 'selected' : '' }}>
                        Impayées
                    </option>
                    <option value="{{ route('admin.factures.index', ['filter' => 'payees']) }}" {{ request('filter') == 'payees' ? 'selected' : '' }}>
                        Payées
                    </option>
                    <option value="{{ route('admin.factures.index', ['filter' => 'mois_en_cours']) }}" {{ request('filter') == 'mois_en_cours' ? 'selected' : '' }}>
                        Ce mois-ci
                    </option>
                </select>
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex items-center space-x-2">
                                <a href="{{ route('admin.factures.show', $facture) }}" class="text-primary hover:text-primary-dark">👁️ Voir</a>
                                <a href="{{ route('admin.factures.edit', $facture) }}" class="text-blue-600 hover:text-blue-800">✏️ Modifier</a>
                                <a href="{{ route('admin.factures.generate-pdf', $facture) }}" target="_blank" class="text-gray-600 hover:text-gray-800">📄 PDF</a>
                                @if($facture->statut != 'payee')
                                <a href="{{ route('admin.paiements.create', ['facture_id' => $facture->id]) }}" class="text-green-600 hover:text-green-800 font-medium">💳 Payer</a>
                                @endif
                            </div>
                        </td>
                    </tr>
